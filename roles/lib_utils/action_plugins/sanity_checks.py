...
class ActionModule(ActionBase):
    ...
    def check_for_removed_vars(self, host):
    ...
    def run_checks(self, host):
        ...
        display.warning("started checks at %s" % current_time.strftime('%Y-%m-%d %H:%M:%S'))
        odt = self.check_openshift_deployment_type(host)
        display.warning("check_openshift_deployment_type %ss" % (dt.fromtimestamp(time.time()) - current_time).total_seconds())
        current_time = dt.fromtimestamp(time.time())
        self.check_whitelisted_registries(host)
        display.warning("check_whitelisted_registries %ss" % (dt.fromtimestamp(time.time()) - current_time).total_seconds())
        current_time = dt.fromtimestamp(time.time())
        self.check_python_version(host, distro)
        display.warning("check_python_version %ss" % (dt.fromtimestamp(time.time()) - current_time).total_seconds())
        current_time = dt.fromtimestamp(time.time())
        self.check_image_tag_format(host, odt)
        display.warning("check_image_tag_format %ss" % (dt.fromtimestamp(time.time()) - current_time).total_seconds())
        current_time = dt.fromtimestamp(time.time())
        self.check_pkg_version_format(host)
        display.warning("check_pkg_version_format %ss" % (dt.fromtimestamp(time.time()) - current_time).total_seconds())
        current_time = dt.fromtimestamp(time.time())
        self.check_release_format(host)
        display.warning("check_release_format %ss" % (dt.fromtimestamp(time.time()) - current_time).total_seconds())
        current_time = dt.fromtimestamp(time.time())
        self.network_plugin_check(host)
        display.warning("network_plugin_check %ss" % (dt.fromtimestamp(time.time()) - current_time).total_seconds())
        current_time = dt.fromtimestamp(time.time())
        self.check_hostname_vars(host)
        display.warning("check_hostname_vars %ss" % (dt.fromtimestamp(time.time()) - current_time).total_seconds())
        current_time = dt.fromtimestamp(time.time())
        self.check_session_auth_secrets(host)
        display.warning("check_session_auth_secrets %ss" % (dt.fromtimestamp(time.time()) - current_time).total_seconds())
        current_time = dt.fromtimestamp(time.time())
        self.check_unsupported_nfs_configs(host)
        display.warning("check_unsupported_nfs_configs %ss" % (dt.fromtimestamp(time.time()) - current_time).total_seconds())
        current_time = dt.fromtimestamp(time.time())
        self.check_htpasswd_provider(host)
        display.warning("check_htpasswd_provider %ss" % (dt.fromtimestamp(time.time()) - current_time).total_seconds())
        current_time = dt.fromtimestamp(time.time())
        self.check_for_removed_vars(host)
        display.warning("check_for_removed_vars %ss" % (dt.fromtimestamp(time.time()) - current_time).total_seconds())
        current_time = dt.fromtimestamp(time.time())
        self.validate_json_format_vars(host)
        display.warning("validate_json_format_vars %ss" % (dt.fromtimestamp(time.time()) - current_time).total_seconds())
        current_time = dt.fromtimestamp(time.time())
        self.check_for_oreg_password(host, odt)
        display.warning("check_for_oreg_password %ss" % (dt.fromtimestamp(time.time()) - current_time).total_seconds())
        current_time = dt.fromtimestamp(time.time())
        raise Exception("done at %s" % dt.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S'))
